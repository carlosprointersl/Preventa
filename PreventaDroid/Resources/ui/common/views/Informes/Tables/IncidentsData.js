/**
 * @fileOverview En este archivo se crean las filas para la tabla de informes. Estas filas contienen los datos relacionados con las incidencias.
 *
...
/**
 * Consulta las incidencias, según los parámetros, y retorna una "sección" válida para una tabla.
 * @class
 * @param {Date} start La fecha de inicio.
 * @param {Date} end La fecha de fin.
 * @return Ti.UI.TableViewSection
 */
function IncidentsData(start, end) {
    /**
     * La varible "Global".
     * @private
     * @type Namespace
     ...
     * @type Function
     */
    var row = require(Global.Path.VIEW + 'Informes/Tables/Rows/IncidentsRow');
     ...
     * @type Object[]
     */
    var incidents = model.select(where + " ORDER BY Fecha DESC");
    //Ti.API.info("****************************** WHERE *******************************\n" + where);
    /**
    ...
    if (incidents.length > 0) {
        for (var i = 0; i < incidents.length; i++) {
            var client = clients.getClient(incidents[i].CodigoCliente);
            incidents[i].Cliente = client.NombreComercial;
            section.add(row(incidents[i]));
        };
